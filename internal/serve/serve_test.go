...
func TestBuildRunnerArgs_Basic(t *testing.T) {
	args := buildRunnerArgs("/ci-cd", "/project", "check", "", "")
	expectedPrefix := []string{"-ExecutionPolicy", "Bypass", "-File", filepath.Join("/ci-cd", "ci-runner.ps1")}
	for i, e := range expectedPrefix {
		if args[i] != e {
			t.Errorf("args[%d] = %q, 期望 %q", i, args[i], e)
		}
	}
	hasAction := false
	hasJson := false
	...
	if !hasAction || !hasJson {
		t.Error("参数应包含 action 和 -Json")
	}
}

func TestBuildRunnerArgs_WithCustom(t *testing.T) {
	args := buildRunnerArgs("/ci-cd", "/project", "build", "npm run build", "--prod")
	foundCmd := false
	foundArgs := false
	for i, a := range args {
		if a == "-CustomCommand" && i+1 < len(args) && args[i+1] == "npm run build" {
			foundCmd = true
		}
		if a == "-CustomArgs" && i+1 < len(args) && args[i+1] == "--prod" {
			foundArgs = true
		}
	}
	if !foundCmd {
		t.Error("应包含 -CustomCommand")
	}
	if !foundArgs {
		t.Error("应包含 -CustomArgs")
	}
}

func TestBuildRunnerArgs_ProjectPath(t *testing.T) {
	args := buildRunnerArgs("/ci-cd", "/my/project/path", "test", "", "")
	foundPath := false
	for i, a := range args {
		if a == "-ProjectPath" && i+1 < len(args) && args[i+1] == "/my/project/path" {
			foundPath = true
		}
	}
	if !foundPath {
		t.Error("应包含正确的 -ProjectPath")
	}
}

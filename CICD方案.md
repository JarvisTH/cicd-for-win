...
> 最后更新：2026-06-23（系统托盘、文件监听、系统通知、跨平台优化、bcrypt 密码哈希、Go/Python 测试报告）  
...
以下功能经过评估后判断与项目定位不符，**明确不做**。保留此记录供后续参考。

| 功能 | 结论 | 理由 |
|------|------|------|
| **事件驱动引擎**（Webhook → 自动触发流水线） | ❌ 不做 | 与「手动触发」定位冲突。如需 Git push 触发，在 Git 仓库配置 post-receive hook 即可，无需内建 Webhook 引擎。引入事件队列将新增 ~1050 行代码，项目膨胀约 20% |
| **多用户 + RBAC** | ❌ 不做 | `localhost:8080` 场景无多用户需求。新增 Session/用户管理/权限校验 ~1600 行代码，项目膨胀约 30%，无实际收益 |

> 两者是 Jenkins/GitLab CI 等**后台服务**需要的功能，与项目定位不符。
